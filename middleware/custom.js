...
let collection = async function (req, res, next) {
	...
	if(!users_array.includes(String(user._id))) return ReE(res, "User does not have permission to read app with id: "+ app_id ? app_id : null);
	...
	if(err) return ReE(res, "err finding channels");
	if(!channels_array) return ReE(res, "channels not found in collection id: "+ collection_id);
...
let channel = async function (req, res, next) {
	...
	if(err) return ReE(res, "err finding channel");

	if(!channel) return ReE(res, "channel not found with id: "+channel_id);

	//get channel this belongs to
	[err, collection] = await to(channel.getCollection());
	if(err) return ReE(res, "err finding collection");
	if(!collection) return ReE(res, "collection not found from channel");
